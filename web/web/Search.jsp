	...
	<div class = "row">
		<div class = "col-4">
			...
			<%
				...
				String kf = request.getParameter("keyField");
				String kw = request.getParameter("keyword");
				// 나중에 지워도 됨
				if(kf == null || kw == null) {
					kf = request.getParameter("field");
					String newkw = request.getParameter("word");
					kw = java.net.URLDecoder.decode(newkw, "UTF-8");
				}

				String sql = "select COUNT(*) from sys.students where " + kf.trim() + " LIKE '%" + kw.trim() + "%'";
								...
								<div class = "col">
									...
									<button id = "mail" class = "btn btn-primary btn-sm active"
											onclick = "location.href='mailto:<%=stMail%>?\n'
													...
													'&body= 지정되지 않은 흡연구역에서 흡연을 하였습니다. 해당과사무실로 찾아오시길 바랍니다.'"><%=stMail%>
									</button>
			...
			<div class = "table-responsive">
				<table id = "findtxt" class = "table">
					...
					<%
						...
						if (keyField == null & keyword == null) {
							keyField = request.getParameter("field");
							String newkeyword = request.getParameter("word");
							keyword = java.net.URLDecoder.decode(newkeyword, "UTF-8");
						}
						ArrayList<stVO> list = dao.findStudent(keyField, keyword);
				...
				<nav aria-label="Page navigation example">
					<ul class="pagination justify-content-center">
						<li class="page-item"><a class="page-link" href = "Search.jsp?pageno=1&studentId=<%=stid%>&filename=<%=filename%>&field=<%=keyField%>&word=<%=keyword%>">[<<]</a></li>
						<li class="page-item"><a class="page-link" href = "Search.jsp?pageno=<%=prev_pageno%>&studentId=<%=stid%>&filename=<%=filename%>&field=<%=keyField%>&word=<%=keyword%>">[<]</a></li>
							<%for (int i = page_sno; i <= page_eno; i++) {%>
							<%if(total_page==0)
								break;%>
									<li class="page-item"><a class="page-link" href = "Search.jsp?pageno=<%=i%>&studentId=<%=stid%>&filename=<%=filename%>&field=<%=keyField%>&word=<%=keyword%>">
								<%if (pageno == i) { %>
								[<%=i %>]
								<%} else { %>
								<%=i %>
						...
						<li class="page-item"><a class="page-link" href = "Search.jsp?pageno=<%=next_pageno%>&studentId=<%=stid%>&filename=<%=filename%>&field=<%=keyField%>&word=<%=keyword%>">[>]</a></li>
						<li class="page-item"><a class="page-link" href = "Search.jsp?pageno=<%=total_page%>&studentId=<%=stid%>&filename=<%=filename%>&field=<%=keyField%>&word=<%=keyword%>">[>>]</a></li>
...
//        document.getElementById("mail").innerHTML = mail;
		...
		window.location.href = "Search.jsp?pageno=" + pageno + "&studentId=" + stid + "&filename=" + filename + "&field=" + keyfield + "&word=" + encodeURI(keyword, "UTF-8");
